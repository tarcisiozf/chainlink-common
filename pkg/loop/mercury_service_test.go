...
import (
	...
	mercuryv1test "github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ext/mercury/v1/test"
	mercuryv2test "github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ext/mercury/v2/test"
	mercuryv3test "github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ext/mercury/v3/test"
	mercuryv4test "github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ext/mercury/v4/test"
...
func TestMercuryV4Service(t *testing.T) {
	...
	}, mercury_common_test.MercuryProvider(lggr), mercuryv4test.DataSource)
	hook := mercuryV4.PluginService.XXXTestHook()
	servicetest.Run(t, mercuryV4)

	t.Run("control", func(t *testing.T) {
		mercury_common_test.MercuryPluginFactory(t, mercuryV4)
	})
...
func TestMercuryV4Service_recovery(t *testing.T) {
	...
	mercury_common_test.MercuryPluginFactory(t, mercury)
}
...
func TestMercuryV3Service(t *testing.T) {
	...
	mercuryV3 := loop.NewMercuryV3Service(lggr, loop.GRPCOpts{}, func() *exec.Cmd {
		return NewHelperProcessCommand(loop.PluginMercuryName, true, 0)
	}, mercury_common_test.MercuryProvider(lggr), mercuryv3test.DataSource)
	hook := mercuryV3.PluginService.XXXTestHook()
	servicetest.Run(t, mercuryV3)

	t.Run("control", func(t *testing.T) {
		mercury_common_test.MercuryPluginFactory(t, mercuryV3)
	})
...
func TestMercuryV1Service(t *testing.T) {
	...
	mercuryV1 := loop.NewMercuryV1Service(lggr, loop.GRPCOpts{}, func() *exec.Cmd {
		return NewHelperProcessCommand(loop.PluginMercuryName, true, 0)
	}, mercury_common_test.MercuryProvider(lggr), mercuryv1test.DataSource)
	hook := mercuryV1.PluginService.XXXTestHook()
	servicetest.Run(t, mercuryV1)

	t.Run("control", func(t *testing.T) {
		mercury_common_test.MercuryPluginFactory(t, mercuryV1)
	})
...
func TestMercuryV2Service(t *testing.T) {
	...
	mercuryV2 := loop.NewMercuryV2Service(lggr, loop.GRPCOpts{}, func() *exec.Cmd {
		return NewHelperProcessCommand(loop.PluginMercuryName, true, 0)
	}, mercury_common_test.MercuryProvider(lggr), mercuryv2test.DataSource)
	hook := mercuryV2.PluginService.XXXTestHook()
	servicetest.Run(t, mercuryV2)

	t.Run("control", func(t *testing.T) {
		mercury_common_test.MercuryPluginFactory(t, mercuryV2)
	})
