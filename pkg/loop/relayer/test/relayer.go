...
import (
	...
	ccipocr3test "github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ext/ccipocr3/test"
	...
	ocr3capabilitytest "github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ext/ocr3capability/test"
	"github.com/smartcontractkit/chainlink-common/pkg/loop/relayer/pluginprovider/ocr2/test"
...
type staticRelayerConfig struct {
	...
	executionProvider      test2.ExecProviderTester
	commitProvider         test2.CommitProviderTester
	configProvider         pluginprovider.ConfigProviderTester
...
func newStaticRelayerConfig(lggr logger.Logger, staticChecks bool) staticRelayerConfig {
	return staticRelayerConfig{
		...
		mercuryProvider:        mercury_common_test.MercuryProvider(lggr),
		executionProvider:      test2.ExecutionProvider(lggr),
		agnosticProvider:       pluginprovider.AgnosticPluginProvider(lggr),
		configProvider:         pluginprovider.ConfigProvider(lggr),
		...
		commitProvider:         test2.CommitProvider(lggr),
		nodeRequest: nodeRequest{
			pageSize:  137,
			pageToken: "",
		},
...
func (s staticRelayer) NewExecutionProvider(ctx context.Context, r types.RelayArgs, p types.PluginArgs) (types.CCIPExecProvider, error) {
	if s.StaticChecks {
		if !equalRelayArgs(r, test2.ExecutionRelayArgs) {
			return nil, fmt.Errorf("expected relay args:\n\t%v\nbut got:\n\t%v", test2.ExecutionRelayArgs, r)
		}
		if !reflect.DeepEqual(test2.ExecutionPluginArgs, p) {
			return nil, fmt.Errorf("expected plugin args %v but got %v", test2.ExecutionPluginArgs, p)
		}
	}
...
func (s staticRelayer) NewCommitProvider(ctx context.Context, r types.RelayArgs, p types.PluginArgs) (types.CCIPCommitProvider, error) {
	if s.StaticChecks {
		if !equalRelayArgs(r, test2.CommitRelayArgs) {
			return nil, fmt.Errorf("expected relay args:\n\t%v\nbut got:\n\t%v", test2.CommitRelayArgs, r)
		}
		if !reflect.DeepEqual(test2.CommitPluginArgs, p) {
			return nil, fmt.Errorf("expected plugin args %v but got %v", test2.CommitPluginArgs, p)
		}
	}
