...
func RegisterPluginProviderServices(s *grpc.Server, provider types.PluginProvider) {
	...
	if provider.ContractReader() != nil {
		pb.RegisterContractReaderServer(s, contractreader2.NewServer(provider.ContractReader()))
	}

	if provider.Codec() != nil {
		codecpb.RegisterCodecServer(s, contractreader2.NewCodecServer(provider.Codec()))
	}
}
