...
func TestOffRampGRPC(t *testing.T) {
	...
	scaffold := looptest.NewGRPCScaffold(t, setupOffRampServer, ccip2.NewOffRampReaderGRPCClient)
...
func roundTripOffRampTests(t *testing.T, client cciptypes.OffRampReader) {
	...
	t.Run("GasPriceEstimator", func(t *testing.T) {
		...
		require.NoError(t, err)
		gasClient, ok := estimator.(*ccip2.ExecGasEstimatorGRPCClient)
...
func setupOffRampServer(t *testing.T, s *grpc.Server, b *loopnet.BrokerExt) *ccip2.OffRampReaderGRPCServer {
	ctx := t.Context()
	offRampProvider, err := ccip2.NewOffRampReaderGRPCServer(ctx, OffRampReader, b)
	require.NoError(t, err)
